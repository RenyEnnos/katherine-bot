...
## 2025-02-12 - [Accessible Transient Feedback]
**Learning:** Dynamically changing the `aria-label` of a button to provide feedback (like "Copied!") is often missed by screen readers because the focus is already on the button. A more reliable pattern is keeping the `aria-label` static (e.g., "Copiar mensagem") while toggling text inside an adjacent, permanently mounted visually hidden (`sr-only`) `div` with `aria-live="polite"`.
**Action:** When a button triggers a temporary state change but the user stays in context, use a hidden `aria-live` region to announce the success/change instead of altering the button's own label.
