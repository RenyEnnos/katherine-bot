...
## 2026-04-27 - [Transient State Accessibility]
**Learning:** Dynamically updating `aria-label` attributes on buttons for transient states (like "Copied!" after clicking) often fails to be announced by screen readers reliably. The recommended approach is to keep the button's `aria-label` static (e.g., "Copiar mensagem") and use a permanently mounted, visually hidden `aria-live="polite"` region adjacent to it that updates its text content when the state changes.
**Action:** Use static `aria-label`s for buttons and adjacent `aria-live` regions for communicating brief, transient success states to assistive technology.
