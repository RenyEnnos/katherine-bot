...
## 2024-05-24 - [Contextual Actions Visibility]
**Learning:** Auxiliary actions (like "Copy") clutter the UI if always visible on every message. However, hiding them completely hurts discoverability and mobile users (no hover).
**Action:** Use `opacity-0 group-hover:opacity-100` for desktop mouse users, but ensure `opacity-100` on mobile (via media queries or default) and `focus-within:opacity-100` for keyboard users.
