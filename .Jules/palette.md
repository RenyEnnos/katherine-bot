...
**Learning:** Using a visually hidden `aria-live` region for transient state feedback (e.g. "Copied!") rather than dynamically updating the `aria-label` of the triggered button provides a much more robust and reliable experience for screen readers. Modifying the `aria-label` abruptly can confuse some assistive technologies.
**Action:** Use an adjacent, permanently mounted `aria-live` region toggling text content for state confirmation instead of updating the primary `aria-label`.
