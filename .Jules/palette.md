...
**Learning:** For accessibility, state confirmations (like 'Copied!') should use a visually hidden 'aria-live' region rather than dynamically updating the 'aria-label' of the triggered button. This ensures reliable screen reader announcements without losing visual tooltips.
**Action:** Use a static 'aria-label' combined with a permanent 'aria-live' container to announce transient feedback like copy confirmation.
