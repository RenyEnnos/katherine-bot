...
class RelationshipManager:
    ...
    def update_relationship(self, relationship: UserRelationship, perception: Dict) -> UserRelationship:
        ...
        valence = get_val("valence", 0.0)
        raw_emotions = perception.get("triggered_emotions")
        if not isinstance(raw_emotions, dict):
            raw_emotions = {}

        def get_emo(key):
            val = raw_emotions.get(key)
            if isinstance(val, bool):
                return 0.0
            if not isinstance(val, (int, float)):
                return 0.0
        ...
        affection_boost = 0.0
        if get_emo("tenderness") > 0.3: affection_boost += 0.03
        if get_emo("joy") > 0.3: affection_boost += 0.01
        if get_emo("gratitude") > 0.3: affection_boost += 0.02
        ...
        tension_spike = 0.0
        if get_emo("anger") > 0.3: tension_spike += 0.1
        if get_emo("disgust") > 0.3: tension_spike += 0.1
