        ...
        <div className={`flex w-full ${isUser ? 'justify-end' : 'justify-start'} mb-6 group`}>
            <div className={`flex max-w-[80%] md:max-w-[70%] ${isUser ? 'flex-row-reverse' : 'flex-row'} gap-4`}>
                ...
                <div className={`flex flex-col gap-1 min-w-0 ${isUser ? 'items-end' : 'items-start'}`}>
                    ...
                    {!isUser && (
                        <div className="opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity px-1">
                            <button
                                onClick={handleCopy}
                                className="flex items-center gap-1.5 text-xs text-gray-500 hover:text-gray-300 transition-colors p-1"
                                aria-label={isCopied ? "Copiado" : "Copiar resposta"}
                                title="Copiar resposta"
                            >
                                {isCopied ? <Check size={14} className="text-green-500" /> : <Copy size={14} />}
                                {isCopied && <span className="text-green-500 font-medium">Copiado!</span>}
                            </button>
                        </div>
                    )}
                </div>
